...
function safeDateFromTimestamp(timestamp){const parsed=toFiniteNumber(timestamp,null); if(parsed===null) return now12(); const d=new Date(parsed); return Number.isNaN(d.getTime())?now12():d.toLocaleString([], {hour12:true});}
function importOldRepairCaptureBackup(raw){
  ...
  const repairs=oldPoints.flatMap((point,idx)=>{if(!point||typeof point!=="object") return []; const lat=toFiniteNumber(point.lat,null); const lng=toFiniteNumber(point.lng,null); if(lat===null||lng===null) return []; const accuracyFt=Math.max(0,Math.round((toFiniteNumber(point.accuracy,0)??0)*3.28084)); const recordId=uid("IMP"); const repairId=String(point.repair_id||`REPAIR-${idx+1}`); const repairType=String(point.repair_type||"Unknown"); const createdAtDisplay=safeDateFromTimestamp(point.timestamp); const record={id:recordId,type:"Repair",title:`Repair - ${repairId}`,status:"LOCKED",time:`${createdAtDisplay} • IMPORTED`,gps:{lat,lng,accuracyFt,time:createdAtDisplay},constants:{...blankConstants()},fields:{repairId,repairType},notes:"Imported from legacy LinerSync Repair Capture backup"}; const asBuilt={id:uid("P"),kind:"Repair",label:`Repair ${repairId}`,x:12+Math.random()*76,y:12+Math.random()*70,color:color("Repair"),recordId}; return [{record,asBuilt}];});
  const perimeterPoints=perimeter.flatMap((point,idx)=>{if(!point||typeof point!=="object") return []; return [{id:uid("P"),kind:"Panel",label:String(point.id||`PER-${String(idx+1).padStart(3,"0")}`),x:12+Math.random()*76,y:12+Math.random()*70,color:color("Panel")}];});
  return {logs:repairs.map(r=>r.record),points:[...repairs.map(r=>r.asBuilt),...perimeterPoints],importedRepairs:repairs.length,importedPerimeter:perimeterPoints.length};
}
...
function App(){
  ...
  function importLegacyRepairCapture(e){const file=e.target.files?.[0]; if(!file) return; const reader=new FileReader(); reader.onload=()=>{try{const parsed=JSON.parse(String(reader.result||"{}")); const imported=importOldRepairCaptureBackup(parsed); if(!imported){setStatus("Import failed: not a legacy Repair Capture backup."); return;} if(!imported.logs.length&&!imported.points.length){setStatus("Import complete: no valid points found."); return;} setLogs(old=>[...imported.logs,...old]); setPoints(old=>[...imported.points,...old]); setStatus(`Imported legacy backup: ${imported.importedRepairs} repairs, ${imported.importedPerimeter} perimeter points.`);}catch{setStatus("Import failed: invalid JSON file.");}}; reader.readAsText(file); e.target.value="";}
  ...
  {tab==="exports"&&<section className="page"><h2>Exports</h2><div className="export-grid"><div className="card"><h3>CSV</h3><button onClick={()=>saveDownload("linersync_logs.csv",makeCsv(logs),"text/csv")}>Download CSV</button></div><div className="card"><h3>KML</h3><button onClick={()=>saveDownload("linersync_points.kml",makeKml(logs),"application/vnd.google-earth.kml+xml")}>Download KML</button></div><div className="card"><h3>JSON Backup</h3><button onClick={()=>saveDownload("linersync_backup.json",JSON.stringify({constants,logs,points},null,2),"application/json")}>Download Backup</button></div><div className="card"><h3>Import Old Repair Capture JSON</h3><input type="file" accept="application/json,.json" onChange={importLegacyRepairCapture}/></div></div></section>}
